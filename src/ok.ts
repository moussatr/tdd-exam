...
function detectStraight(cards: Card[]): number | null {
  ...
  if (JSON.stringify(ranks) === JSON.stringify([14, 5, 4, 3, 2])) return 5;
  return null;
}
...
export function evaluate5(cards: Card[]): HandResult {
  ...
  // Trie les groupes par fréquence (brelan > paire) puis par rang (As > Roi)
  const rankGroups = [...counts.entries()].sort(
    (a, b) => b[1] - a[1] || b[0] - a[0]
  );

  // Reconstruit chosen5 en respectant l'ordre d'importance (tie-break)
  const groupedCards = rankGroups.flatMap(([rank]) => 
    sorted.filter(c => c.rank === rank)
  );

  const flush = isFlush(sorted);
  const straightHigh = detectStraight(sorted);

  // Gère l'ordre visuel de la quinte blanche (Wheel : 5, 4, 3, 2, A)
  let straightCards = sorted;
  if (straightHigh === 5 && sorted[0].rank === 14) {
    straightCards = [...sorted.slice(1), sorted[0]];
  }
